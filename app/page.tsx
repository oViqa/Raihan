...
export default function Home() {
  ...
  // Typed state variables using imported types
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="container mx-auto px-4 relative z-10 h-full flex flex-col justify-center items-center text-center">
        ...
        <p className="text-xl md:text-2xl mb-10 text-[#f0ece2] max-w-2xl" style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}>
          {isArabic 
            ? 'اكتشف منتجات طبيعية أصيلة من جبال الأطلس في المغرب محصودة من قبل أجيال من المزارعين المحليين'
        ...
        <Link
          ...
          style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}
        >
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center justify-center mb-3">
          ...
          <h2 className="text-4xl font-bold text-[#4a5a2b] moroccan-heading" style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}>
            {t('our_products')}
          </h2>
        </div>
        <p className="text-[#8e846b] text-xl max-w-3xl mx-auto mt-3" style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}>
          {isArabic 
            ? 'كنوز مختارة يدويًا من أفضل المحاصيل المغربية'
        ...
        >
          {t('all_products')}
          <span className={`transform group-hover:translate-x-2 transition-transform ${isArabic ? 'mr-2 rotate-180' : 'ml-2'}`}>→</span>
        </Link>
        </div>
      </div>
      </section>
      ...
      <section className="py-20 bg-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="mb-16 text-center">
            ...
            <p className="text-[#8e846b] text-xl max-w-3xl mx-auto mt-4" style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}>
              {isArabic 
                ? 'فئات مختارة من أفضل المنتجات الطبيعية المغربية'
              ...
              return (
                ...
                >
                  ...
                  <div className="p-8 relative z-10 flex flex-col items-center text-center h-full">
                    ...
                    <h3 className="text-2xl font-bold text-[#4a5a2b] mb-3 group-hover:text-[#6b7f3e] transition-colors" style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}>
                      {category.name}
                    </h3>
                    
                    <p className="text-[#8e846b] mb-6 flex-grow" style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}>
                      {category.description || (isArabic 
                        ? `منتجات ${category.name} أصيلة من جبال الأطلس`
                        : `Authentique ${category.name.toLowerCase()} des montagnes de l'Atlas`
                      )}
                    </p>
                    
                    <span className="inline-flex items-center justify-center bg-white/80 text-[#c17f24] font-medium py-2 px-4 rounded-full shadow-sm backdrop-blur-sm group-hover:bg-[#6b7f3e] group-hover:text-white transition-all duration-300" style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}>
                      {t('discover')} <span className={`transform group-hover:translate-x-1 transition-transform ${isArabic ? 'mr-1 rotate-180' : 'ml-1'}`}>→</span>
                    </span>
                  </div>
          ...
          {categories.length > 3 && (
            <div className="text-center mt-12">
              ...
              >
                ...
                <span className={`${isArabic ? 'mr-2 rotate-180' : 'ml-2'}`}>→</span>
              </Link>
            </div>
          )}
        </div>
      ...
      <footer className="bg-[#323232] text-white py-12">
        <div className="container mx-auto px-4">
          <div className="text-center">
            ...
            <p className="mb-4 text-[#d3c8ab]" style={isArabic ? { fontFamily: "'Noto Sans Arabic', sans-serif" } : {}}>
              {isArabic ? 'كنوز طبيعية من جبال الأطلس' : 'Trésors naturels des montagnes de l\'Atlas'}
            </p>
            <p className="text-[#8e846b]">© {new Date().getFullYear()} Raihan. {isArabic ? 'جميع الحقوق محفوظة' : 'Tous droits réservés.'}</p>
            <div className="mt-6 space-x-4">
              ...
              <Link
                ...
                className="text-[#d3c8ab] hover:text-white text-sm"
              >
                {isArabic ? 'بوابة المشرف' : 'Portail Admin'}
              </Link>
            </div>
          </div>
        </div>
